...
### 3. JVM 内存为什么要分成新生代，老年代，持久代。新生代中为什么要分为 Eden 和 Survivor。一次完整的 GC 流程是怎样的，对象如何晋升到老年代。
...
#### 4. 为什么要分为 Eden 和 Survivor ?为什么要设置两个 Survivor 区？
...
### 5. JMM ( Java ) 内存模型的相关知识了解多少，比如重排序，内存屏障，happen-before，主内存，工作内存。
