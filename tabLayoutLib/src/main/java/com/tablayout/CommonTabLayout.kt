...
class CommonTabLayout @JvmOverloads constructor(
    ...
    private fun addTab(position: Int, tabView: View) {
        ...
        /** 每一个Tab的布局参数  */
        var lp_tab =
            if (mTabSpaceEqual) LinearLayout.LayoutParams(0, LayoutParams.MATCH_PARENT, 1.0f) else LinearLayout.LayoutParams(
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        } else { /* mRectPaint.setColor(mIndicatorColor);
                calcIndicatorRect();
    ...
    fun setMsgMargin(position: Int, leftPadding: Float, bottomPadding: Float) {
        ...
        if (tipView != null) {
            ...
            if (mIconGravity == Gravity.TOP || mIconGravity == Gravity.BOTTOM) {
                ...
                lp.topMargin =
                    if (mHeight > 0) (mHeight - textHeight - iconH - margin).toInt() / 2 - dp2px(bottomPadding) else dp2px(bottomPadding)
            } else {
                lp.leftMargin = dp2px(leftPadding)
                lp.topMargin =
                    if (mHeight > 0) (mHeight - Math.max(textHeight, iconH)).toInt() / 2 - dp2px(bottomPadding) else dp2px(
                        bottomPadding
                    )
            }
